...
api_key = "AIzaSyDJXM76PBgc66ambPjg6b8Z8TItLIqcvP8" 
...
def prepare_data_for_mysql(data):
    new_data = []
    for key, value in data.items():
        if key == "Dislikes":
            ...
            new_data.append(','.join(map(str, value)))
            ...
            new_data.append(value)
    print(f"Transformed data: {tuple(new_data)}")
...
if selected == "Extract & Transform":
    ...
    def insert_into_channels():
        ...
        for i in collections.find({"Channel_name" : user_inp},{'_id' : 0}):
            transformed_data = prepare_data_for_mysql(i)
        
            channel_id = transformed_data[0]
            mycursor.execute("SELECT COUNT(*) FROM channels WHERE channel_id = %s", (channel_id,))
            count = mycursor.fetchone()[0]
            if count == 0:
            # Only insert if the channel_id does not exist
              try:
                mycursor.execute(query, transformed_data)
                mydb.commit()
              except Exception as e:
                st.error(f"Failed to insert data: {transformed_data}. Error: {e}")
                st.write(f"Original Data: {i}")  # Add this line to display the original record
                st.write(f"Transformed Data: {transformed_data}")
...
if selected == "View":
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        ...
        mycursor.execute("""
             SELECT a.video_id AS Video_id, a.title AS Video_Title, COALESCE(b.Total_Comments, 0) AS Total_Comments
             FROM videos AS a
             LEFT JOIN (
            ...
            ORDER BY COALESCE(b.Total_Comments, 0) DESC
            """)
        df = pd.DataFrame(mycursor.fetchall(), columns=mycursor.column_names)
        st.write(df)
        ...
        mycursor.execute("""SELECT title AS Title, likes AS Likes_Count, dislikes AS Dislikes_Count
                        FROM videos
